CLASS LHC_ZI_PURITYCODEALL_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE PurityCodeALL\_PurityCode
          ENTITIES_UPDATE FOR UPDATE PurityCode.
ENDCLASS.

CLASS LHC_ZI_PURITYCODEALL_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE ZI_PurityCode\_PurityCodeText,
          text_for_existing_entity TYPE TABLE FOR CREATE ZI_PurityCode\_PurityCodeText,
          text_update              TYPE TABLE FOR UPDATE ZI_PurityCodeText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK ZI_PurityCode\_PurityCodeText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Puritycode = <target>-%KEY-Puritycode
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF ZI_PurityCodeALL_S
      ENTITY PurityCode
        CREATE BY \_PurityCodeText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF ZI_PurityCodeALL_S
        ENTITY PurityCode BY \_PurityCodeText
